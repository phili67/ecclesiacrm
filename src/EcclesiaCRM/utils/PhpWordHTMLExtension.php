...
class PhpWordHTMLExtension
{
     ...
     */
    public static function addHtml(\PhpOffice\PhpWord\Element\Section $element, string $html, bool $fullHTML = false, bool $preserveWhiteSpace = true, array $options = null )
    {
        /*
    ...
    protected static function parseNode($node, $element, $styles = array(), $data = array())
    {
        ...
        if (isset($nodes[$node->nodeName])) {
            ...
            $method = "parse{$method}";
            $newElement = call_user_func_array(array('PhpOffice\PhpWord\Shared\PhpWordHTMLExtension', $method), array_values($arguments));
    ...
    protected static function parseHeading($element, &$styles, $argument1)
    {
        ...
        LoggerUtils::getAppLogger()->info("head : ".print_r($styles['head1'],true));
    ...
    protected static function parseText($node, $element, &$styles)
    {
        ...
        LoggerUtils::getAppLogger()->info($node->nodeValue);
