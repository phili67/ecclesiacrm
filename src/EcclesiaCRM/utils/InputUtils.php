...
{    
    ...
    // Liste corrigée : tag => attributs autorisés
    // iframe retiré (vecteur XSS majeur, voir explication précédente)
    // video conservé mais avec attributs strictement limités
    private static $AllowedHTMLTags = [
        ...
        // 'iframe'  => volontairement absent
    ];

    private static $AllowedSchemes = ['http', 'https', 'mailto'];

    /**
     * Remplace l'ancien FilterHTML basé sur strip_tags().
     * Filtre les tags ET les attributs, valide les schémas d'URI.
     */
    public static function FilterHTML($sInput)
    {
        ...
        return self::sanitizeHtml($sInput);
    }
    ...
    private static function cleanNode(\DOMNode $node, \DOMDocument $dom): void
    {
        ...
        foreach ($children as $child) {
            ...
            if (!array_key_exists($tagName, self::$AllowedHTMLTags)) {
                // Balise interdite : on nettoie récursivement puis on "déroule"
                // (on garde le contenu texte/enfants, comportement proche de strip_tags)
            ...
            // Balise autorisée : filtrage des attributs
            $allowedAttrs = self::$AllowedHTMLTags[$tagName];
            $attributesToRemove = [];
            ...
            foreach (iterator_to_array($child->attributes) as $attr) {
                ...
                if (in_array($attrName, ['href', 'src', 'poster'], true)) {
                    if (!self::isSafeUri($attr->value)) {
                        $attributesToRemove[] = $attr->name;
                    }
                }
            }
    ...
    private static function isSafeUri(string $uri): bool
    {
        ...
        if (!preg_match('#^([a-zA-Z][a-zA-Z0-9+.\-]*):#', $uri, $matches)) {
            return true; // relative, pas de schéma => ok
        }

        return in_array(strtolower($matches[1]), self::$AllowedSchemes, true);
    }
    ...
    {
        $clean = self::sanitizeHtml(trim($sInput));
        return mb_substr($clean, 0, $size);
    }
