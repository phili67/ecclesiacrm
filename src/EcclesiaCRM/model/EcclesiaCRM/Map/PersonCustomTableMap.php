...
class PersonCustomTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'EcclesiaCRM.Map.PersonCustomTableMap';
     ...
     */
    const TABLE_NAME = 'person_custom';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\EcclesiaCRM\\PersonCustom';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'EcclesiaCRM.PersonCustom';
     ...
     */
    const COL_PER_ID = 'person_custom.per_ID';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('PerId', ),
        self::TYPE_CAMELNAME     => array('perId', ),
        self::TYPE_COLNAME       => array(PersonCustomTableMap::COL_PER_ID, ),
        self::TYPE_FIELDNAME     => array('per_ID', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('PerId' => 0, ),
        self::TYPE_CAMELNAME     => array('perId' => 0, ),
        self::TYPE_COLNAME       => array(PersonCustomTableMap::COL_PER_ID => 0, ),
        self::TYPE_FIELDNAME     => array('per_ID' => 0, ),
        self::TYPE_NUM           => array(0, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\EcclesiaCRM\\PersonCustom');
  ...
  array (
    ...
    public static function getPrimaryKeyHashFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        return null === $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('PerId', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('PerId', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('PerId', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('PerId', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('PerId', TableMap::TYPE_PHPNAME, $indexType)];
    }
    ...
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if (null !== ($obj = PersonCustomTableMap::getInstanceFromPool($key))) {
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(PersonCustomTableMap::COL_PER_ID);
        } else {
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(PersonCustomTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(PersonCustomTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new PersonCustomTableMap());
        }
    }
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \EcclesiaCRM\PersonCustom) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(PersonCustomTableMap::DATABASE_NAME);
            $criteria->add(PersonCustomTableMap::COL_PER_ID, (array) $values, Criteria::IN);
        }
     ...
     *
     * @param mixed               $criteria Criteria or PersonCustom object containing data that is used to create the INSERT statement.
...
} // PersonCustomTableMap
// This is the static code needed to register the TableMap for this table with the main Propel class.
//
PersonCustomTableMap::buildTableMap();
