...
class ListOptionTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'EcclesiaCRM.Map.ListOptionTableMap';
     ...
     */
    const OM_CLASS = '\\EcclesiaCRM\\ListOption';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'EcclesiaCRM.ListOption';
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\EcclesiaCRM\\ListOption');
    ...
    {
        $this->addRelation('Group', '\\EcclesiaCRM\\Group', RelationMap::ONE_TO_MANY, array (
  0 =>
  array (
    0 => ':grp_RoleListID',
    1 => ':lst_ID',
  ...
  array (
     ...
     * and findPk*() calls.
     *
     * @param \EcclesiaCRM\ListOption $obj A \EcclesiaCRM\ListOption object.
    ...
    public static function addInstanceToPool($obj, $key = null)
    {
        if (Propel::isInstancePoolingEnabled()) {
            if (null === $key) {
                $key = serialize([(null === $obj->getId() || is_scalar($obj->getId()) || is_callable([$obj->getId(), '__toString']) ? (string) $obj->getId() : $obj->getId()), (null === $obj->getOptionId() || is_scalar($obj->getOptionId()) || is_callable([$obj->getOptionId(), '__toString']) ? (string) $obj->getOptionId() : $obj->getOptionId())]);
            } // if key === null
     ...
     *
     * @param mixed $value A \EcclesiaCRM\ListOption object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \EcclesiaCRM\ListOption) {
                $key = serialize([(null === $value->getId() || is_scalar($value->getId()) || is_callable([$value->getId(), '__toString']) ? (string) $value->getId() : $value->getId()), (null === $value->getOptionId() || is_scalar($value->getOptionId()) || is_callable([$value->getOptionId(), '__toString']) ? (string) $value->getOptionId() : $value->getOptionId())]);
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \EcclesiaCRM\ListOption object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    public static function getPrimaryKeyHashFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        return serialize([(null === $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)]), (null === $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('OptionId', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('OptionId', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('OptionId', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('OptionId', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('OptionId', TableMap::TYPE_PHPNAME, $indexType)])]);
    }
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \EcclesiaCRM\ListOption) { // it's a model object
