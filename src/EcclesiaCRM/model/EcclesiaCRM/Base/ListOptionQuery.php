 ...
 *
 * This table stores the options for most of the drop down lists in EcclesiaCRM, including person classifications, family roles, group types, group roles, group-specific property types, and custom field value lists.
 *
 ...
 *
 * @method     \EcclesiaCRM\GroupQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class ListOptionQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \EcclesiaCRM\Base\ListOptionQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\EcclesiaCRM\\ListOption', $modelAlias = null)
    {
     ...
     * </code>
     *
     * @param array[$lst_ID, $lst_OptionID] $key Primary key to use for the query
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = ListOptionTableMap::getInstanceFromPool(serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]))))) {
    ...
    protected function findPkSimple($key, ConnectionInterface $con)
    {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            ListOptionTableMap::addInstanceToPool($obj, serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]));
        }
        $stmt->closeCursor();
     ...
     *
     * @param \EcclesiaCRM\Group|ObjectCollection $group the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
    ...
    public function filterByGroup($group, $comparison = null)
    {
        if ($group instanceof \EcclesiaCRM\Group) {
            return $this
                ->addUsingAlias(ListOptionTableMap::COL_LST_ID, $group->getRoleListId(), $comparison)
                ->addUsingAlias(ListOptionTableMap::COL_LST_OPTIONID, $group->getType(), $comparison);
        } else {
            throw new PropelException('filterByGroup() only accepts arguments of type \EcclesiaCRM\Group');
        }
    }
