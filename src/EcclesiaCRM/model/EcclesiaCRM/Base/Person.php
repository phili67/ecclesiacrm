...
use EcclesiaCRM\PersonProperty as ChildPersonProperty;
...
use EcclesiaCRM\WhyCame as ChildWhyCame;
use EcclesiaCRM\WhyCameQuery as ChildWhyCameQuery;
...
use EcclesiaCRM\Map\WhyCameTableMap;
...
abstract class Person implements ActiveRecordInterface
{
    ...
    /**
     * @var        ObjectCollection|ChildProperty[] Cross Collection to store aggregation of ChildProperty objects.
     */
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->propertiesScheduledForDeletion !== null) {
                if (!$this->propertiesScheduledForDeletion->isEmpty()) {
                    ...
                    foreach ($this->propertiesScheduledForDeletion as $entry) {
                        ...
                        $entryPk[0] = $entry->getProId();
                        $pks[] = $entryPk;
                    }

                    \EcclesiaCRM\PersonPropertyQuery::create()
            ...
            if ($this->whyCamesScheduledForDeletion !== null) {
                if (!$this->whyCamesScheduledForDeletion->isEmpty()) {
                    \EcclesiaCRM\WhyCameQuery::create()
    ...
    public function initWhyCames($overrideExisting = true)
    {
        ...
        $this->collWhyCames->setModel('\EcclesiaCRM\WhyCame');
    }
    ...
    public function initPersonProperties($overrideExisting = true)
    {
        ...
        $this->collPersonProperties->setModel('\EcclesiaCRM\PersonProperty');
    }
    ...
    public function setPersonProperties(Collection $personProperties, ConnectionInterface $con = null)
    {
        ...
        foreach ($personPropertiesToDelete as $personPropertyRemoved) {
            $personPropertyRemoved->setPerson(null);
        }
     ...
     * @return ObjectCollection|ChildPersonProperty[] List of ChildPersonProperty objects
     */
    public function getPersonPropertiesJoinProperty(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildPersonPropertyQuery::create(null, $criteria);
    ...
    public function initProperties()
    {
        ...
        $this->collProperties->setModel('\EcclesiaCRM\Property');
    }
    ...
    /**
     * Gets a collection of ChildProperty objects related by a many-to-many relationship
    ...
    /**
     * Sets a collection of Property objects related by a many-to-many relationship
     * to the current object by way of the record2property_r2p cross-reference table.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
    ...
    /**
     * Gets the number of Property objects related by a many-to-many relationship
     * to the current object by way of the record2property_r2p cross-reference table.
     *
     * @param      Criteria $criteria Optional query object to filter the query
     ...
     * @param      ConnectionInterface $con Optional connection object
     *
     * @return int the number of related Property objects
     */
    ...
    /**
     * Associate a ChildProperty to this object
     * through the record2property_r2p cross reference table.
     *
     * @param ChildProperty $property
     * @return ChildPerson The current object (for fluent API support)
     */
    public function addProperty(ChildProperty $property)
    {
    ...
    protected function doAddProperty(ChildProperty $property)
    {
        ...
        $personProperty->setProperty($property);
        ...
        if (!$property->isPeopleLoaded()) {
            $property->initPeople();
            $property->getPeople()->push($this);
        } elseif (!$property->getPeople()->contains($this)) {
            $property->getPeople()->push($this);
        }
    ...
    public function removeProperty(ChildProperty $property)
    {
        if ($this->getProperties()->contains($property)) {
            ...
            if ($property->isPeopleLoaded()) {
                ...
                $property->getPeople()->removeObject($this);
            }
