...
class SystemConfig
{
  ...
  {
    return array(
        "sLogLevel" => new ConfigItem(4, "sLogLevel", "choice", "100", gettext("Event Log severity to write, used by ORM and App Logs"), "", json_encode(SystemConfig::getMonoLogLevels())),
        ...
        "iSessionTimeout" => new ConfigItem(9, "iSessionTimeout", "number", "3600", gettext("Session timeout length in seconds. Set to zero to disable session timeouts.")),
        ...
        "iMinPasswordChange" => new ConfigItem(14, "iMinPasswordChange", "number", "4", gettext("Minimum amount that a new password must differ from the old one (# of characters changed). Set to zero to disable this feature")),
        ...
        "iMaxFailedLogins" => new ConfigItem(16, "iMaxFailedLogins", "number", "5", gettext("Maximum number of failed logins to allow before a user account is locked.. Once the maximum has been reached, an administrator must re-enable the account.. This feature helps to protect against automated password guessing attacks.. Set to zero to disable this feature.")),
        "iPDFOutputType" => new ConfigItem(20, "iPDFOutputType", "choice", "1", gettext("PDF handling mode.. 1 = Save File dialog. 2 = Open in current browser window"),"",'{"Choices":["1","2"]}'),
        ...
        "bShowFamilyData" => new ConfigItem(33, "bShowFamilyData", "boolean", "1", gettext("Unavailable person info inherited from assigned family for display?. This option causes certain info from a person's assigned family record to be. displayed IF the corresponding info has NOT been entered for that person. ")),
        ...
        "sNominatimLink" => new ConfigItem(41, "sNominatimLink", "text", "https://nominatim.openstreetmap.org", gettext("Link of the nominatim server : https://nominatim.openstreetmap.org") , "https://OpenStreetMap.openstreetmap.org"),
        ...
        "iMapZoom" => new ConfigItem(10001, "iMapZoom", "number", "12", gettext("Google/OpenStreetMap/BingMaps Maps Zoom")),
        "iLittleMapZoom" => new ConfigItem(10002, "iLittleMapZoom", "number", "15", gettext("Google/OpenStreetMap/BingMaps Litle Maps Zoom")),
        ...
        "sMapProvider" => new ConfigItem(56, "sMapProvider", "choice", "OpenStreetMap", gettext("Select GeoCoder Provider") , "https://github.com/geocoder-php/Geocoder/blob/3.x/README.md#address-based-providers", '{"Choices":["OpenStreetMap", "GoogleMaps", "BingMaps"]}'),
        ...
        "sDatePickerPlaceHolder" => new ConfigItem(109, "sDatePickerPlaceHolder", "choice", "yyyy-mm-dd", gettext("For defining the date in Date-Picker, per default : yyyy-mm-dd, In French : dd/mm/yyyy for example."),'','{"Choices":["yyyy-mm-dd","dd/mm/yyyy","mm-dd-yyyy","mm/dd/yyyy"]}'),
        "sDatePickerFormat" => new ConfigItem(110, "sDatePickerFormat", "choice", "Y-m-d", gettext("For defining the date in Date-Picker, per default : Y-m-d, In French : d/m/Y for example."),'','{"Choices":["Y-m-d","d/m/Y","m-d-Y","m/d/Y"]}'),
        "bRegistered" => new ConfigItem(999, "bRegistered", "boolean", "0", gettext("EcclesiaCRM has been registered.  The EcclesiaCRM team uses registration information to track usage.  This information is kept confidential and never released or sold.  If this field is true the registration option in the admin menu changes to update registration.")),
        ...
        "iSearchIncludePersonsMax" => new ConfigItem(2027, "iSearchIncludePersonsMax", "text", "15", gettext("Maximum number of People")),
        "iSearchIncludeFamiliesMax" => new ConfigItem(2028, "iSearchIncludeFamiliesMax", "text", "15", gettext("Maximum number of Families")),
        "iSearchIncludeFamilyHOHMax" => new ConfigItem(2029, "iSearchIncludeFamilyHOHMax", "text", "15", gettext("Maximum number of Family H.O.H Names")),
        "iSearchIncludeGroupsMax" => new ConfigItem(2030, "iSearchIncludeGroupsMax", "text", "15", gettext("Maximum number of Groups")),
        "iSearchIncludeDepositsMax" => new ConfigItem(2031, "iSearchIncludeDepositsMax", "text", "5", gettext("Maximum number of Deposits")),
        "iSearchIncludePaymentsMax" => new ConfigItem(2032, "iSearchIncludePaymentsMax", "text", "5", gettext("Maximum number of Payments")),
        "iSearchIncludeAddressesMax" => new ConfigItem(2033, "iSearchIncludeAddressesMax", "text", "15", gettext("Maximum number of Addresses")),
        "bSearchIncludePastoralCare" => new ConfigItem(2060, "bSearchIncludePastoralCare", "boolean", "1", gettext("Search Pastoral Care")),
        "iSearchIncludePastoralCareMax" => new ConfigItem(2061, "iSearchIncludePastoralCareMax", "text", "5", gettext("Maximum number of Pastoral Care results")),
        ...
        "bStateUnusefull" => new ConfigItem(2050, "bStateUnusefull", "boolean", "0", gettext("Hide dropdown states when they are unusefull")),
