            ...
            $("#container").html(`
                <div class="d-flex flex-column align-items-center justify-content-center" style="min-height: 40vh;">
                    <div class="spinner-border text-primary mb-4" role="status" style="width: 4rem; height: 4rem;">
                        <span class="sr-only">${i18next.t('Loading...')}</span>
                    </div>
                    <h2 class="text-primary font-weight-bold mb-2">${i18next.t('Loading data...')}</h2>
                ...
                for (i = 0; i < len; i++) {
                            ...
                            <div class="col-lg-4 col-lg-mailchimp mb-3">
                             <b><i class="fas fa-info-circle"></i> ${i18next.t('Details')}</b><br>
                              <table class="table table-sm table-borderless mb-0">
                                                                <tr><td><i class="fas fa-heading text-primary mr-2"></i> ${i18next.t('Subject')}</td><td><span class="text-primary">"${list.campaign_defaults.subject}"</span></td></tr>
                                                                <tr><td><i class="fas fa-users text-secondary mr-2"></i> ${i18next.t('Members:')}</td><td><span class="badge badge-secondary">${list.stats.member_count}</span></td></tr>
                                                                <tr><td><i class="fas fa-user-slash text-warning mr-2"></i> ${i18next.t('Unsubscribed count:')}</td><td><span class="badge badge-warning text-dark">${list.stats.unsubscribe_count}</span></td></tr>
                                                                <tr><td><i class="fas fa-user-minus text-warning mr-2"></i> ${i18next.t('Unsubscribed count since last send:')}</td><td><span class="badge badge-warning text-dark">${list.stats.unsubscribe_count_since_send}</span></td></tr>
                                                                <tr><td><i class="fas fa-broom text-danger mr-2"></i> ${i18next.t('Cleaned count:')}</td><td><span class="badge badge-danger">${list.stats.cleaned_count}</span></td></tr>
                                                                <tr><td><i class="fas fa-broom text-danger mr-2"></i> ${i18next.t('Cleaned count since last send:')}</td><td><span class="badge badge-danger">${list.stats.cleaned_count_since_send}</span></td></tr>
                              </table>
                            </div>
                    ...
                    for (j = 0; j < lenCampaigns; j++) {
                        ...
                        listViews += `<tr>
                            ...
                            <td class="align-middle"><span class="badge badge-${badgeClass}">${i18next.t(status)}</span></td>
                            </tr>`;
                    }
                                ...
                                for (k = 0; k < lenTags; k++) {
                                    listViews += `
                                        <div class="badge badge-light border text-dark d-flex align-items-center px-2 py-2 mb-2 shadow-sm w-100" style="font-size:1rem; justify-content: start;">
                                            <a class="delete-tag ml-3" data-id="${tags[k].id}" data-listid="${data.MailChimpCampaigns[i][send_campaigns].id}" title="${i18next.t('Delete')}">
                            ...
                            <div class="card-body">
                                <div class="alert alert-danger d-flex align-items-center mb-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="mb-1">${i18next.t('Please update the')} <b>MailChimp</b> ${i18next.t('API key in Setting->')} <a href="${window.CRM.root}/v2/systemsettings/integration" class="alert-link"><i class="fas fa-cog mr-1"></i>${i18next.t('Edit General Settings')}</a>.</p>
                                        <p class="mb-1">${i18next.t('Then update')} <b>sMailChimpApiKey</b>.</p>
                                        <p class="mb-0">${i18next.t('For more info see our ')}<a href="${window.CRM.getSupportURL}" class="alert-link" target="_blank"><i class="fab fa-mailchimp mr-1"></i>MailChimp ${i18next.t('support docs.')}</a></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="container-fluid px-0">
                            <div class="mb-3 row align-items-center">
                                <label for="ListTitle" class="col-sm-4 col-form-label text-end">
                                    <span class="text-danger">*</span> <i class="fas fa-heading text-primary mr-1"></i> ${i18next.t('List Title')} :
                                </label>
                            ...
                            <div class="mb-3 row align-items-center">
                                <label for="Subject" class="col-sm-4 col-form-label text-end">
                                    <span class="text-danger">*</span> <i class="fas fa-envelope text-primary mr-1"></i> ${i18next.t('Subject')} :
                                </label>
                                <div class="col-sm-8 d-flex align-items-center">
                            ...
                            <div class="mb-3 row align-items-center">
                                <label for="PermissionReminder" class="col-sm-4 col-form-label text-end">
                                    <span class="text-danger">*</span> <i class="fas fa-info-circle text-primary mr-1"></i> ${i18next.t('Permission Reminder')} :
                                </label>
                                <div class="col-sm-8 d-flex align-items-center">
                            ...
                            <div class="mb-3 row align-items-center">
                                <div class="offset-sm-4 col-sm-8 d-flex align-items-center">
                                    <div class="form-check">
                                        ...
                                        <label class="form-check-label" for="ArchiveBars"><i class="fas fa-archive text-secondary mr-1"></i> ${i18next.t('Archive Bars')}</label>
                                    </div>
                                </div>
                            </div>
                            <div class="mb-3 row align-items-center">
                                <label class="col-sm-4 col-form-label text-end">
                                    <span class="text-danger">*</span> <i class="fas fa-user-shield text-primary mr-1"></i> ${i18next.t('Status')}
                                </label>
                                <div class="col-sm-8 d-flex align-items-center">
                                    <div class="form-check mr-3">
                                        <input class="form-check-input" type="radio" name="Status" id="StatusPrv" value="prv" checked>
                                        <label class="form-check-label" for="StatusPrv"><i class="fas fa-lock mr-1"></i> ${i18next.t('Private')}</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="Status" id="StatusPub" value="pub">
                                        <label class="form-check-label" for="StatusPub"><i class="fas fa-globe mr-1"></i> ${i18next.t('Public')}</label>
                                    </div>
                                </div>
                            </div>
                        </div>
