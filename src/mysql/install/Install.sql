...
INSERT INTO `query_qry` (`qry_ID`, `qry_SQL`, `qry_Name`, `qry_Description`, `qry_Count`, `qry_Type_ID`) VALUES
  ...
  (25, 'SELECT per_ID as AddToCart, CONCAT(''<a href=PersonView.php?PersonID='',per_ID,''>'',per_FirstName,'' '',per_LastName,''</a>'') AS Name, per_DateDeactivated as ''GDPR'' FROM person_per LEFT JOIN person2volunteeropp_p2vo ON per_id = p2vo_per_ID WHERE p2vo_vol_ID = ~volopp~ ORDER BY per_LastName', 'Volunteers', 'Find volunteers for a particular opportunity', 1, 6),
...
CREATE TABLE `plugin` (
  ...
  `plgn_default_color` enum('bg-gradient-blue text-white', 'bg-gradient-indigo text-white', 'bg-gradient-navy text-white', 'bg-gradient-maroon text-white', 'bg-gradient-purple text-white', 'bg-gradient-pink text-white', 'bg-gradient-red text-white', 'bg-gradient-orange text-white', 'bg-gradient-yellow text-white', 'bg-gradient-lime text-white', 'bg-gradient-green text-white', 'bg-gradient-teal text-white', 'bg-gradient-cyan text-white', 'bg-gradient-gray text-black') NOT NULL default 'bg-gradient-blue text-white' COMMENT 'Default Background dashboard color',
...
CREATE TABLE `plugin_user_role` (
    ...
    `plgn_usr_rl_position` mediumint(9) unsigned NOT NULL default '0' COMMENT 'position on the dashboard',
