        ...
        <div class="col-md-3 mb-3">
            <div class="card card-outline card-success text-center h-100 mb-0">
                <div class="card-body py-3">
                    ...
                    <div class="text-muted small"><?= dgettext("messages-MailChimp", "Opened") ?> · <?= $openRate ?>%</div>
                </div>
            </div>
        </div>
        <div class="col-md-3 mb-3">
            <div class="card card-outline card-info text-center h-100 mb-0">
                <div class="card-body py-3">
                    ...
                    <div class="text-muted small"><?= dgettext("messages-MailChimp", "Clicked") ?> · <?= $clickRate ?>%</div>
                </div>
            </div>
        </div>
        <div class="col-md-3 mb-3">
            <div class="card card-outline card-warning text-center h-100 mb-0">
                <div class="card-body py-3">
                    ...
                    <div class="text-muted small"><?= dgettext("messages-MailChimp", "Bounced") ?></div>
                </div>
            </div>
        </div>
        <div class="col-md-3 mb-3">
            <div class="card card-outline card-danger text-center h-100 mb-0">
                <div class="card-body py-3">
                    ...
                    <div class="text-muted small"><?= dgettext("messages-MailChimp", "Unsubscribed") ?></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card-body py-3 d-flex flex-wrap align-items-center justify-content-between">
        ...
        <div class="d-flex flex-wrap">
            <button class="btn btn-success mr-2 mb-2 mb-md-0" id="saveCampaign" data-listid="<?= $list_id ?>" <?= (($campaign['status'] == "sent") ? 'disabled' : '') ?>>
                <i class="fas fa-save mr-1"></i><?= dgettext("messages-MailChimp", 'Save Campaign') ?>
            </button>
            <button id="sendCampaign" class="btn btn-outline-primary mr-2 mb-2 mb-md-0 <?= (($campaign['status'] == "sent" || $campaign['status'] == "schedule") ? 'hidden' : '') ?>" data-listid="<?= $list_id ?>">
                <i class="far fa-paper-plane mr-1"></i><?= dgettext("messages-MailChimp", 'Send') ?>
            </button>
            <button id="deleteCampaign" class="btn btn-outline-danger mb-2 mb-md-0" data-listid="<?= $list_id ?>">
                <i class="fas fa-trash-alt mr-1"></i><?= dgettext("messages-MailChimp", 'Delete') ?>
            </button>
        </div>
    </div>
</div>
...
<div class="row">
    <div class="col-lg-9">
        <div class="card card-outline card-primary shadow-sm mb-3">
            <div class="card-header">
                <h3 class="card-title"><i class="far fa-file-alt mr-1"></i><?= dgettext("messages-MailChimp", 'Mail Subject') ?> & <?= dgettext("messages-MailChimp", 'Content') ?></h3>
            </div>
            <div class="card-body">
                <div class="form-group row">
                    <label class="col-md-2 col-form-label"><span class="text-danger">*</span> <?= dgettext("messages-MailChimp", 'Subject') ?></label>
                    <div class="col-md-10">
                        <input type="text" id="CampaignSubject" placeholder="<?= dgettext("messages-MailChimp", 'Your Mail Subject') ?>" maxlength="100" class="form-control form-control-sm" value="<?= $campaign['settings']['subject_line'] ?>">
                    </div>
                </div>
                <div class="alert alert-light border mb-3">
                    <i class="fas fa-info-circle text-success mr-1"></i>
                    <?= dgettext("messages-MailChimp", 'You can use the button "Merge Tags" below, to customize your content') ?>
    ...
    <div class="col-lg-3">
        <div class="card card-outline card-secondary shadow-sm mb-3">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-tags mr-1"></i><?= dgettext("messages-MailChimp", 'Tags') ?></h3>
        ...
        <div class="card card-outline card-warning shadow-sm">
                ...
                <h3 class="card-title mb-0">
                    <div class="custom-control custom-switch">
                        ...
                        <label class="custom-control-label" for="checkboxaCampaignSchedule">
                            <i class="fas <?= ($campaign['status'] == "schedule") ? 'fa-calendar-check' : 'fa-calendar-times' ?> mr-1"></i><?= dgettext("messages-MailChimp", 'Schedule') ?>
                        </label>
                    </div>
                </h3>
                <div class="card-tools">
            ...
            <div class="card-body">
                <div class="alert alert-warning">
                    <i class="fas fa-exclamation-triangle mr-1"></i><?= dgettext("messages-MailChimp", "You've first to create a content below to schedule a campaign.") ?><br>
                    <i class="fas fa-exclamation-triangle mr-1"></i><?= dgettext("messages-MailChimp", "Campaigns may only be scheduled to send on the quarter-hour (:00, :15, :30, :45).") ?>
                </div>
                <div class="form-group row">
                    <label class="col-md-3 col-form-label"><span class="text-danger">*</span> <?= dgettext("messages-MailChimp", 'Date') ?></label>
                    <div class="col-md-9">
                ...
                <div class="form-group row mb-0">
                    <label class="col-md-3 col-form-label"><span class="text-danger">*</span> <?= dgettext("messages-MailChimp", 'Time') ?></label>
                    <div class="col-md-9">
                ...
                <small class="text-muted d-block mt-2"><?= dgettext("messages-MailChimp", 'To validate, save your campaign with the <b>"Save Campaign"</b> button over.') ?></small>
            </div>
        </div>
    </div>
</div>
...
<script src="<?= $sRootPath ?>/Plugins/MailChimp/skin/js/Campaign.js"></script>
