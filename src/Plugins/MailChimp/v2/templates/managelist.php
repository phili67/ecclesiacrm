        ...
        <div class="d-flex flex-wrap">
            <div class="btn-group mr-2 mb-2 mb-md-0">
                <button class="btn btn-success CreateCampaign" id="CreateCampaign" data-listid="<?= $listId ?>" data-id="-1" data-name="">
                    <i class="fas fa-envelope-open-text mr-1"></i><?= dgettext("messages-MailChimp", 'Create a Campaign') ?>
                </button>
                <button type="button" id="addCreateCampaignTagDrop" class="btn btn-success dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-expanded="false">
                    <span class="sr-only"><?= dgettext("messages-MailChimp", 'Toggle Dropdown') ?></span>
                </button>
            ...
            <button id="deleteAllSubScribers" class="btn btn-outline-warning mr-2 mb-2 mb-md-0" data-listid="<?= $listId ?>">
                <i class="far fa-trash-alt mr-1"></i><?= dgettext("messages-MailChimp", 'Delete All Subscribers') ?>
            </button>
            <button class="btn btn-outline-primary mr-2 mb-2 mb-md-0" id="modifyList" data-name="<?= $list['name'] ?>" data-subject="<?= $list['campaign_defaults']['subject'] ?>" data-permissionreminder="<?= $list['permission_reminder'] ?>">
                <i class="fas fa-pencil-alt mr-1"></i><?= dgettext("messages-MailChimp", 'Modify Properties') ?>
            </button>
        ...
        <div class="col-lg-9">
            <div class="card card-outline card-primary shadow-sm">
                <div class="card-header">
                    <h3 class="card-title"><i class="fas fa-users mr-1"></i><?= dgettext("messages-MailChimp", 'Subscribers') ?></h3>
                </div>
                <div class="card-body">
                    <div class="alert alert-info mb-3"><i class="fas fa-info-circle mr-1"></i>
                        <?= dgettext("messages-MailChimp", "To add all the newsletter users, type <b>NewLetter</b> in the search field, to add all members of the CRM, use <b>*</b>") ?>
                    </div>

                    <div class="row align-items-center mb-3">
                        <div class="col-md-2">
                            <label class="mb-0 font-weight-bold"><?= dgettext("messages-MailChimp", 'Add') ?></label>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-4">
                            <small class="text-warning"><i class="fas fa-tag mr-1"></i><?= dgettext("messages-MailChimp", 'Keywords') ?> : *, <?= dgettext("messages-MailChimp", 'Persons') ?>, <?= dgettext("messages-MailChimp", 'Families') ?>, <?= dgettext("messages-MailChimp", 'newsletter') ?>...</small>
                        </div>
                    </div>
    ...
    <div class="card card-outline card-danger shadow-sm">
        <div class="card-body">
            <div class="alert alert-danger mb-0">
                <h5><i class="fas fa-ban mr-1"></i>MailChimp <?= dgettext("messages-MailChimp", 'is not configured') ?></h5>
                <?= dgettext("messages-MailChimp", 'Please update the') ?> MailChimp <?= dgettext("messages-MailChimp", 'API key in Setting->') ?>
                <a href="<?= $sRootPath ?>/v2/systemsettings/integration"><?= dgettext("messages-MailChimp", 'Edit General Settings') ?></a>,
                <?= dgettext("messages-MailChimp", 'then update') ?> sMailChimpApiKey.
                <?= dgettext("messages-MailChimp", 'For more info see our ') ?>
                <a href="<?= $getSupportURL ?>" target="_blank">MailChimp <?= dgettext("messages-MailChimp", 'support docs.') ?></a>
            </div>
        </div>
    </div>
...
<script src="<?= $sRootPath ?>/Plugins/MailChimp/skin/js/ManageList.js"></script>
