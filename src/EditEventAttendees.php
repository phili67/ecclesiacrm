...
$sPageTitle = _('Church Event Editor');
...
<div class='box-header'>
    <h3 class='box-title'><?= _('Attendees for Event ID:').' '.$EventID ?></h3>
</div>
...
<div class='box'>
  ...
  <tr class="TableHeader">
    <th width="35%"><strong><?= _('Name') ?></strong></td>
    <th width="25%"><strong><?= _('Email') ?></strong></td>
    <th width="25%"><strong><?= _('Home Phone') ?></strong></td>
    <th width="15%" nowrap><strong><?= _('Action') ?></strong></td>
  </tr>
  </thead>
  <tbody>
<?php
  ...
  foreach ($ormOpps as $ormOpp) {
   ...
   $sEmail = MiscUtils::SelectWhichInfo($person->getEmail(), (!empty($fam))?$fam->getEmail():"", false);?>
    ...
    <td colspan="1" align="center">
       <a class="btn btn-danger DeleleAttendees" data-personid="<?=$person->getId()?>" data-eventid="<?= $EventID ?>"> <?= _("Delete") ?></a>
     </td>
    </tr>
    <?php
  }
    ...
    ?>
<tr><td colspan="4" align="center"><?= _('No Attendees Assigned to Event') ?></td></tr>
<?php
}
...
<div class="row">
<div class="col-sm-6">
    <a id="DeleleAllAttendees" class="btn btn-danger <?= ($numAttRows == 0)?"disabled":"" ?>" data-eventid="<?= $EventID ?>"><?= _("Delele All Attendees") ?></a>
</div>
<div class="col-sm-6">
...
<?php } ?>
      <button type="submit" name="Action" title="<?=_('Make Check-out') ?>" data-tooltip <?= ($numAttRows-$countCheckout>0)?'value="'._('Make Check-out').'"':"" ?> class="btn btn-<?= ($numAttRows-$countCheckout == 0)?"default disabled":"success" ?>">        
