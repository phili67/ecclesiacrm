...
function getSupportedLocales()
{
    ...
    $res = '<select id="sLanguage" name="sLanguage" class="form-control select2" aria-describedby="sChurchNameHelp" required>';
    $first = true;
...
function select_Timezone($selected = '') {
    ...
    $select= '<select id="sTimeZone" name="sTimeZone" class="form-control select2" aria-describedby="sTimeZoneHelp" required>';
...
<h1 class="text-center mb-4">🎉 Welcome to the <b>CRM</b><?=SystemService::getPackageMainVersion() ?> Setup Wizard!</h1>
<p class="text-center mb-4">Let's get your CRM up and running! Just follow these simple steps below. Required fields are marked with an asterisk (*).<br><span style="font-size:0.95em;color:#888;">(You’re in good hands!)</span></p>
<form id="setup-form" autocomplete="on">
    <div id="wizard">
        ...
        <section>
            ...
            <span style="font-size:0.95em;color:#888;">(If something’s missing, don’t panic! You can fix it later.)</span></p>
        ...
        <section>
            ...
            <span style="font-size:0.95em;color:#888;">(Don’t worry, you can change this later if needed.)</span></p>
                ...
                <section>
                    ...
                    <div class="card-body">
                        <div class="alert alert-success" id="prerequisites-war">
                                ...
                                <span style="font-size:0.95em;color:#888;">(And don’t forget to treat yourself to a coffee – you’ve earned it!)</span></p>
                        </div>
                    </div>
                    </div>
                </section>
...
<script>
// Personnalisation des boutons Next/Previous du wizard après génération
document.addEventListener('DOMContentLoaded', function() {
    function styleWizardButtons() {
        // Next
        document.querySelectorAll('a[href="#next"]').forEach(function(btn) {
            btn.classList.add('btn', 'btn-primary', 'rounded-pill', 'px-4', 'me-2');
            btn.innerHTML = '<i class="fa fa-arrow-right"></i> Next';
        });
        // Previous
        document.querySelectorAll('a[href="#previous"]').forEach(function(btn) {
            btn.classList.add('btn', 'btn-secondary', 'rounded-pill', 'px-4', 'ms-2');
            btn.innerHTML = '<i class="fa fa-arrow-left"></i> Previous';
        });
        // Finish
        document.querySelectorAll('a[href="#finish"]').forEach(function(btn) {
            btn.classList.add('btn', 'btn-success', 'rounded-pill', 'px-4');
            btn.innerHTML = '<i class="fa fa-check"></i> Finish';
        });
    }
    // Initial styling
    setTimeout(styleWizardButtons, 300);
    // Re-style on step change (jquery.steps events)
    $("#wizard").on("stepChanged", function() { setTimeout(styleWizardButtons, 100); });
});
</script>
