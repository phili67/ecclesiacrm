...
?>
  <div class="row">
    <div id="right-buttons" class="btn-group" role="group">
      <button type="button" id="verify" class="btn btn-sm" data-toggle="modal" data-target="#confirm-Verify"><div class="btn-txt"><?=_("Confirm")?></div><i class="fa fa-check fa-5x"></i>  </button>
    </div>
  </div>
  <div class="card card-info" id="verifyBox">
    <div class="panel-body">
      ...
      <div class="text-muted font-bold m-b-xs">
        ...
        ?>
            <i class="fa fa-fw fa-heart" title="<?= _("Wedding Date")?>"></i><?= $family->getWeddingDate()->format(SystemConfig::getValue("sDateFormatLong")) ?><br/>
        <?php
          }
        ?>

        <i class="fa fa-fw fa-newspaper-o" title="<?= _("Send Newsletter")?>"></i><?= _($family->getSendNewsletter()) ?><br/>
      </div>
    </div>
    ...
    <div class="card card-solid">
      <div class="card-header">
        ...
        <h3 class="card-title"><?= _("Family Member(s)")?></h3>
      </div>
      <div class="row">
              ...
              <div class="card-body box-profile">
                ...
                <ul class="list-group list-group-unbordered">
                  <li class="list-group-item">
                    ...
                    <i class="fa fa-fw fa-phone" title="<?= _("Home Phone")?>"></i>(H) <?= $person->getHomePhone() ?><br/>
                      <?php }  if (!empty($person->getWorkPhone()))  { ?>
                    <i class="fa fa-fw fa-briefcase" title="<?= _("Work Phone")?>"></i>(W) <?= $person->getWorkPhone() ?><br/>
                      <?php }  if (!empty($person->getCellPhone()))  { ?>
                    <i class="fa fa-fw fa-mobile" title="<?= _("Mobile Phone")?>"></i>(M) <?= $person->getCellPhone() ?><br/>
                      <?php }  if (!empty($person->getEmail()))  { ?>
                    <i class="fa fa-fw fa-envelope" title="<?= _("Email")?>"></i>(H) <?= $person->getEmail() ?><br/>
                  ...
                  <li class="list-group-item">
                    ...
                    ?>
                        <b><?= $groupMembership->getGroup()->getName() ?></b>: <span class="pull-right"><?= _($listOption) ?></span><br/>
                    <?php
                        }
                    }
                    ?>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button id="onlineVerifyCancelBtn" type="button" class="btn btn-default" data-dismiss="modal"><?= _("Cancel") ?></button>
          <button id="onlineVerifyBtn" class="btn btn-success"><?= _("Verify") ?></button>
          <a href="<?= ChurchMetaData::getChurchWebSite() ?>" id="onlineVerifySiteBtn" class="btn btn-success"><?= _("Visit our Site") ?></a>
        </div>
      </div>
    </div>
  </div>
